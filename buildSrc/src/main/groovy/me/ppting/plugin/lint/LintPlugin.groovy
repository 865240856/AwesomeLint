...
class LintPlugin implements Plugin<Project> {
    ...
    def fileNameFix = [".java", ".xml",".kt"] as String[]
    ...
    void apply(Project project) {
        ...
        project.task("lintCheck")  {
            doLast{
                if (project.lintConfig != null && project.lintConfig.isOpenLint){
                     ...
                     * 输出TXT格式的报告
                     */
                    File lintResult = new File("lint-check-result.log")
                     ...
                     */
                    try {
                        println("开始进行 Lint run")
                        println("断言"+cl.flags.reporters.isEmpty())
